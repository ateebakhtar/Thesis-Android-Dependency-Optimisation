...
# Paths and constants
# os.environ["PATH"] += ":/Users/ateeb/Library/Android/sdk/cmdline-tools/latest/bin"
...
def extract_dependency_tree(output_csv_path="dependency_tree.csv"):
    """
    Extract the dependency tree of the project and save it to a CSV file.
    """
    try:
        os.chdir(PROJECT_DIR)
        # Run the Gradle dependencies task
        result = subprocess.run(
        ...
        if result.returncode != 0:
            raise Exception(f"Error extracting dependencies: {result.stderr.strip()}")

        # Parse the output
        dependency_lines = result.stdout.strip().splitlines()

        # Filter and clean dependency lines
        dependencies = []
        for line in dependency_lines:
            line = line.strip()
            if line and not line.startswith("task") and not line.startswith("Deprecated") and not line.startswith("+"):
                dependencies.append(line)

        # Save to CSV
        with open(output_csv_path, mode='w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            writer.writerow(["Dependency Tree"])
            for dependency in dependencies:
                writer.writerow([dependency])

        print(f"Dependency tree successfully saved to {output_csv_path}")

    except Exception as e:
        print(f"Error extracting dependency tree: {str(e)}")
    finally:
        os.chdir("..")  # Return to the original directory
...
def main():
    ...
    extract_dependency_tree("dependency_tree.csv")
